...
use crate::library::definitions::*;

pub(crate) fn perform_dct(image: &DynamicImage) -> Vec<f64> {
    let mut raw_bytes = bytify(&image);

    let dimension = (raw_bytes.len() as f64).sqrt() as usize;
    if RESIZE_IMAGE_X != dimension {
        panic!("actual x: {}, RESIZE_IMAGE_X: {}", dimension, RESIZE_IMAGE_X);
    }

    if RESIZE_IMAGE_Y != dimension {
        panic!()
    }
    ...
    raw_bytes.chunks_exact_mut(dimension).for_each(|mut row| {
    ...
    raw_bytes.chunks_exact_mut(dimension).for_each(|mut col| {
...
pub(crate) fn inverse_dct(raw_bytes: &[f64]) -> image::DynamicImage {
    let dimension = (raw_bytes.len() as f64).sqrt() as usize;
    ...
    //perform round 1
    raw_bytes.chunks_mut(dimension).for_each(|mut row| {
        dct.process_dct3(&mut row);
    });
    ...
    raw_bytes.chunks_mut(dimension).for_each(|mut col| {
        dct.process_dct3(&mut col);
    });
...
fn imagify(raw_bytes: &[f64]) -> DynamicImage {
    ...
    let buf = DynamicImage::ImageLuma8(buf);
