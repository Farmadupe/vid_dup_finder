...
impl GuiEntryState {
    pub fn new(thunk: ResolutionThunk, single_mode: bool, thumb_choice: ThumbChoice, zoom: ZoomState) -> Self {
        let info = thunk
            .entries()
            .into_iter()
            .map(|src_path| (src_path, thunk.stats(src_path), thunk.hash(src_path)))
            .collect::<Vec<_>>();
    ...
    pub fn distance(&self) -> String {
        match self.thunk.distance() {
            Some(distance) => format!(
                "Spatial: {:0.2}, Temporal: {:0.2}",
                distance.spatial as f64 / crate::library::definitions::HASH_DISTANCE_SCALING_FACTOR as f64,
                distance.temporal as f64 / crate::library::definitions::HASH_DISTANCE_SCALING_FACTOR as f64
            ),
            None => "?????".to_string(),
        }
    }
    ...
    fn render_entry(&self, i: usize) -> gtk::Box {
        ...
        let win_row = gtk::Box::new(gtk::Orientation::Horizontal, 5);
    ...
    pub fn vlc_all(&self) {
        let mut path_iter = self
            ...
            .into_iter()
            .map(|x| x.display())
            //.map(|x| shell_words::quote(&*x).to_string());
            .map(|x| x.to_string());

        //let first_arg = shell_words::quote(&path_iter.next().unwrap()).to_string();
        let first_arg = path_iter.next().unwrap();

        let input_slave_arg = "--input-slave=".to_string() + &path_iter.collect::<Vec<_>>().join("#");

        let mut command = std::process::Command::new("vlc");
        let command = command.arg(first_arg).arg(&input_slave_arg);

        if let Err(e) = command.spawn() {
            warn!("Failed to start vlc at {}: {}", input_slave_arg, e);
        }
    }
